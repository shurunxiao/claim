      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="lookReport(scope.row, scope.$index)">编辑</el-button>
            <el-button type="text" size="small" v-show="loginInfo.roleStr === '1'" @click="allotReport(scope.row, scope.$index)">分配</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      queryTask(){
        var url = window.SERVE_URl + 'getTaskList';
        var param = {
        ...
        this.$http.post(url, param).then(res => {
          ...
          this.taskList.forEach(item => {
            ...
            try {
              // item.startTime = item.startTime ? new Date(item.startTime).format() : '';
      ...
      allotReport(task, index){
        this.$router.push({name: 'ReportCreate', params: {reportNo: task.businessKey, handleBy: task.handleBy}});
      },
